...
module overmind::NonFungibleToken {
    ...
    */
    public fun mint_nft(
        ...
        assert!( coin::value(payment_coin)>= 1000000000,EInsufficientPayment);
        let _change:Coin<SUI>;
        if (value>1000000000){
            let amount_to = value - 1000000000;
            _change = coin::split(payment_coin,amount_to,ctx);
        }else{
            _change = coin::zero(ctx);
        };
        balance::join(coin::balance_mut(payment_coin),coin::into_balance(_change));
        
        let coin_balance = coin::balance_mut(payment_coin);
        // coin::take(coin_balance,1,ctx);
        // let coin_balance = coin::balance_mut(payment_coin);

        let paid = balance::split(coin_balance, 1000000000);
        
        // let increase = coin::mint_balance(tresury_cap,1);

        // let paid = coin::into_balance(_change);
        balance::join(&mut minter_cap.sales,paid);
        // let {id, sales} = *minter_cap;
    ...
    */
    ...
    public fun combine_nfts(
        ...
        // let mut s = string::utf8(string::bytes(&nft1.name));
        let name1 = nft1.name;
        let name2 = nft2.name;
        nft1.description = string::utf8(b"");
        string::insert(&mut nft1.description,0,string::utf8(b"Combined NFT of "));
        string::append(&mut nft1.description,name1);
        string::append(&mut nft1.description, string::utf8(b" and "));
        string::append(&mut nft1.description ,name2);


        string::append(&mut nft1.name ,string::utf8(b" + "));
        string::append(&mut nft1.name ,nft2.name);

        // let mut s = string::utf8(b"Combined NFT of ");
        



        // let des1 = nft1.description;

        // let comb_desc= vector::empty<u8>();
        // vector::append(&mut comb_desc,b"Combined NFT of ");
        // vector::append(&mut comb_desc,string::bytes(&de));
        // vector::append(&mut comb_desc,b" and ");
        // vector::append(&mut comb_desc,string::bytes(nft2.description));

        // let comb_desc: &mut vector<u8> = b"";
        // vector::push_back(&mut comb_desc,b"Combined NFT of ");
        // vector::push_back(&mut comb_desc,nft1.description);
        // vector::push_back(&mut comb_desc,b" and ");
        // vector::push_back(&mut comb_desc,nft2.description);
        // let comb_desc = b"";
        let combine_nft = NonFungibleToken{
            id:object::new(ctx),
            name:nft1.name,
        ...
        event::emit(NonFungibleTokenCombined{ nft1_id:object::id(&nft1),nft2_id:object::id(&nft2),new_nft_id:object::id(&combine_nft)});
    ...
    */
    public fun withdraw_sales(
        ...
        let withdraw = coin::take(&mut minter_cap.sales,balance,ctx);
        event::emit(SalesWithdrawn{amount:balance});
        withdraw
        // transfer::public_transfer(withdraw)
